        ...
        <div v-else class="posts-list">
          ...
          >
            ...
            <div class="post-content">
              <p class="post-text">{{ post.content.substring(0, 30) }}{{ post.content.length > 30 ? '...' : '' }}</p>
            ...
            <div class="post-footer">
              <div class="post-info">
                <span class="post-author">
                  <el-avatar :size="24" :src="post.authorAvatar">{{ post.author.substring(0, 1) }}</el-avatar>
                  {{ post.author }}
                </span>
              ...
              <div class="post-actions">
                ...
                >
                  <el-icon :color="post.isLiked ? '#f56c6c' : ''">
            ...
            <div v-if="post.category === 'review'" class="rating-container">
    ...
    >
      <el-form :model="postForm" label-width="80px" label-position="top">
        ...
        <el-form-item label="图片">
          ...
          >
            <div v-if="postForm.imageUrl" class="image-preview">
              <el-image :src="postForm.imageUrl" fit="cover" class="preview-image" />
        ...
        <el-form-item v-if="postForm.category === 'review'" label="评分">
      ...
      <div v-if="currentPost.id" class="post-detail">
        <div class="post-detail-header">
          <div class="detail-author-info">
            <el-avatar :size="36" :src="currentPost.authorAvatar">{{ currentPost.author?.substring(0, 1) }}</el-avatar>
            <div class="detail-meta">
              <span class="detail-author">{{ currentPost.author }}</span>
              <span class="detail-time">{{ formatDateTime(currentPost.createTime) }}</span>
            </div>
          </div>
        ...
        <div class="post-detail-content">
          ...
          <div v-if="currentPost.imageUrl" class="detail-image-container">
            <el-image
                :src="currentPost.imageUrl"
                fit="contain"
                class="detail-image"
                :preview-src-list="[currentPost.imageUrl]"
            />
          </div>
        </div>

        <div v-if="currentPost.category === 'review'" class="detail-rating">
...
const categoryOptions = [
  ...
  { label: '书籍评价', value: 'review' },
  { label: '推荐交流', value: 'recommendation' },
  { label: '求助问答', value: 'question' }
]

// 判断用户是否可以管理帖子
const canManagePost = (post) => {
  return post.author === username || role === 'staff' || role === 'admin'
}
...
const fetchPosts = async () => {
  ...
  try {
    ...
    const response = await getAllPosts(params.category)
...
const submitPost = async () => {
  ...
  try {
    ...
    if (isEditing.value) {
      response = await updatePost(postForm.id, postData)
    } else {
